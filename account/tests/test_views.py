...
class ResetPasswordViewTests(TestCase):
    def setUp(self):
        self.user = CustomUser.objects.create_user(username= 'test',email='test@example.com', password='oldpassword123')
        self.uidb64 = urlsafe_base64_encode(force_bytes(self.user.pk))
        self.token = default_token_generator.make_token(self.user)
        self.valid_url = reverse('account:reset-password', args=[self.uidb64, self.token])
        self.invalid_url = reverse('account:reset-password', args=['invaliduid', 'invalidtoken'])

    def test_get_valid_reset_password_page(self):
        response = self.client.get(self.valid_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'account/reset-password.html')

    def test_get_invalid_reset_password_page(self):
        response = self.client.get(self.invalid_url)
        self.assertRedirects(response, reverse('account:login'))

    def test_post_valid_reset_password(self):
